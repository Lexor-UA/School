...
import 'package:swimming_school_app/features/admin/presentation/widgets/admin_activity_log_sheet.dart';
import 'package:swimming_school_app/features/admin/presentation/widgets/admin_quick_action_card.dart';
...
class _AdminMainState extends ConsumerState<AdminMain> {
  ...
  Widget _buildLivePulseBar(AdminDashboardState state) {
    ...
    // 2. Скільки клієнтів займаються зараз (суворо у воді)
    final int clientsValue = state.ongoingClientsCount;

    // 3. Скільки тренерів працюють прямо зараз (проводять заняття на зміні)
    final int coachesValue = state.ongoingCoachesCount;
  ...
  void _showRecentActionsSheet(BuildContext context, List<ActivityLog> actions) {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => AdminActivityLogSheet(actions: actions),
    );
  }
}
